...
bool GenHPVM::runOnModule(Module &M) {
  ...
  Value *init_caller = *VI->user_begin();
  while (!isa<Instruction>(*init_caller)) {
    init_caller = *init_caller->user_begin();
  }
  Instruction *I = cast<Instruction>(init_caller);
  ...
  Value *cleanup_caller = *VC->user_begin();
  while (!isa<Instruction>(*cleanup_caller)) {
    cleanup_caller = *cleanup_caller->user_begin();
  }
  I = cast<Instruction>(cleanup_caller);
  printTimerSet(I);
